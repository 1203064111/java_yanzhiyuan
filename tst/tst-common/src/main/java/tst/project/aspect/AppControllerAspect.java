...
//		}
	...
	public Object appControllerAspectAround(ProceedingJoinPoint pjp) throws Throwable {
		...
		try {
			...
			if(url.indexOf("registerMember")>=0){//注册接口 要判断短时间不让重复提交
			...
			if(isVerToken){
				...
				String requrest_key="member_id"+member_id+":"+request.getRequestURI();
				if(requrest_key.indexOf("get")<0){//查询的不限制
					String count=RedisUtils.getKey(requrest_key);
					if("1".equals(count)){
						BaseBean baseBean=new BaseBean();
						baseBean.setStatus("error");
			...
			if(isVerSystemToken){
				...
				String requrest_key="account_id1"+account_id+":"+request.getRequestURI();
				if(requrest_key.indexOf("get")<0){//查询的不限制
					String count=RedisUtils.getKey(requrest_key);
					if("1".equals(count)){
						BaseBean baseBean=new BaseBean();
